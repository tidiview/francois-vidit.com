...
ところがその中には美しさどころか、どんな物もはいっていず、ただ幽冥界の、というより{r}正(しょう)真(しん)正(しょう)銘(めい){/r}の{r}地獄(ステュクス){/r}の眠りだけが、蓋を取られると見る見る立ち昇ってプシケーにかかり、{r}昏(こん)迷(めい){/r}の{r}靄(もや){/r}でひたひたと{r}手(て)肢(あし)中(じゆう){/r}を取り巻いてしまいました。
